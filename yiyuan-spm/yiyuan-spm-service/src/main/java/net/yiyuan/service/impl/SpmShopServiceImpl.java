...
@Slf4j
@Service
public class SpmShopServiceImpl extends JoinServiceImpl
    ...
    @Override
    public List
            <SpmShopQueryVO> list(SpmShopListDTO request) throws Exception {

        SpmShop po = new SpmShop();
        BeanUtilsPlus.copy(request, po);
        JoinLambdaWrapper
                <SpmShop> wrapper = new JoinLambdaWrapper<>(po);
        List
                <SpmShopQueryVO> voList = spmShopMapper.joinSelectList(wrapper, SpmShopQueryVO.class);
    ...
    @Override
    public Page
            <SpmShopQueryVO> page(SpmShopPageDTO request) throws Exception {
        SpmShop po = new SpmShop();
        BeanUtilsPlus.copy(request, po);
        JoinLambdaWrapper
                <SpmShop> wrapper = new JoinLambdaWrapper<>(po);
        Page
                <SpmShopQueryVO> voPage =
    ...
    public SpmShopQueryVO details(SpmShop request) throws Exception {
        JoinLambdaWrapper
                <SpmShop> wrapper = new JoinLambdaWrapper<>(request);
            SpmShopQueryVO voBean = spmShopMapper.joinSelectOne(wrapper, SpmShopQueryVO.class);
        return voBean;
    }
