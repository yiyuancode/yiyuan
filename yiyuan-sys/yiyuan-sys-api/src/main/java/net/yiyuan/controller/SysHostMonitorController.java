...
public class SysHostMonitorController {
   ...
   * @date 2023-09-18
   */
  @Description("系统管理/服务器监控数据采集管理/查询服务器监控数据采集")
  @SaCheckPermission(
  ...
  public CommonResult<List<SysHostMonitorQueryVO>> list(SysHostMonitorListDTO request)
      throws Exception {
    return CommonResult.success(sysHostMonitorService.list(request), "查询服务器监控数据采集列表成功");
  }
  ...
  public CommonResult<Page<SysHostMonitorQueryVO>> page(SysHostMonitorPageDTO request)
      throws Exception {
    return CommonResult.success(sysHostMonitorService.page(request), "分页查询服务器监控数据采集成功");
  }
  ...
  public CommonResult<SysHostMonitorQueryVO> details(@PathVariable("id") @NotBlank String id)
      throws Exception {
    return CommonResult.success(sysHostMonitorService.details(id), "查询服务器监控数据采集详情成功");
  }
   ...
   * @date 2023-09-18
   */
  @Description("系统管理/服务器监控数据采集管理/删除服务器监控数据采集")
  @SaCheckPermission(
  ...
  public CommonResult<String> delete(
    ...
    if (sysHostMonitorService.delete(ids)) {
      return CommonResult.success(null, "删除服务器监控数据采集成功");
    } else {
      return CommonResult.failed("删除服务器监控数据采集失败");
    }
  }
   ...
   * @date 2023-09-18
   */
  @Description("系统管理/服务器监控数据采集管理/编辑服务器监控数据采集")
  @SaCheckPermission(
  ...
  public CommonResult<String> edit(@RequestBody @Validated SysHostMonitorEditDTO request)
    ...
    if (sysHostMonitorService.edit(request)) {
      return CommonResult.success(null, "修改服务器监控数据采集成功");
    } else {
      return CommonResult.failed("修改服务器监控数据采集失败");
    }
  }
   ...
   * @date 2023-09-18
   */
  @Description("系统管理/服务器监控数据采集管理/新增服务器监控数据采集")
  @SaCheckPermission(
  ...
  public CommonResult<String> add(@RequestBody @Validated SysHostMonitorAddDTO request)
    ...
    if (sysHostMonitorService.add(request)) {
      return CommonResult.success(null, "新增服务器监控数据采集成功");
    } else {
      return CommonResult.failed("新增服务器监控数据采集失败");
    }
  }
}
