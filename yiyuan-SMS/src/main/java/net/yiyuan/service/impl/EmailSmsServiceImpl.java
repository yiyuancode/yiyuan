...
public class EmailSmsServiceImpl implements SmsAndEmailService {
  ...
  @Resource private MailConfig mailConfig;
  ...
  public boolean checkSmsCode(String phoneOrEmail, String code) {
    ...
    String scode =
        smsRedisService.get(SmsRedisService.REDIS_KEY_EMAIL_PERMISSION, phoneOrEmail, String.class);
  ...
  private MimeMessage buildMail(Session session, Mailreciever mailreciever) throws Exception {
    ...
    message.setRecipients(
        MimeMessage.RecipientType.TO, InternetAddress.parse(mailreciever.getEmails()));
