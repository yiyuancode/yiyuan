package net.yiyuan.filecontext;

import lombok.extern.slf4j.Slf4j;
import net.yiyuan.config.FileOperatorConfigInter;
import net.yiyuan.enums.FileOperatorTypeEnum;
...
public class FileOperatorContext {
     ...
     * 文件操作器集合
     */
    private final Map<FileOperatorTypeEnum, FileOperatorInter> operatorMap = new ConcurrentHashMap<>(2);
    ...
    private void init() {
        ...
        Arrays.stream(FileOperatorTypeEnum.values())
                ...
                .findFirst()
                .orElseThrow(() -> new FileOperatorException(String.format("文件服务 | 当前指定的激活文件操作器名[%s]参数错误", activeTypeName)));
        //实例化已支持的文件操作器
        try {
            for (FileOperatorTypeEnum typeEnum : FileOperatorTypeEnum.values()) {
                ...
                Optional<? extends FileOperatorConfigInter> optionalConfigInter = operatorConfigs.values().stream().findFirst();
