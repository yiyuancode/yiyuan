...
import net.yiyuan.utils.KeyValue;
...
public class Enums {
    ...
    public static <T extends Enum<T>> T getByKeyValue(Class<T> cls, KeyValue<String, ?> kv) {
        ...
        throw CustomUnifiedException.aom(String.format("Enum类：%s 不存在指定键值 | %s", cls.getSimpleName(), kv));
    }
     ...
     */
    @SneakyThrows
    public static <T extends Enum<T>> Optional<T> getOptionalByKeyValue(Class<T> cls,
                                                                        KeyValue<String, ?> kv) {
        PropertyDescriptor propertyDescriptor = BeanUtils.getPropertyDescriptor(cls, kv.getKey());
        if (propertyDescriptor != null) {
            for (T one : EnumUtils.getEnumList(cls)) {
