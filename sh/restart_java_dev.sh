...
docker run -d -p 50032:8085  --privileged=true  -v /install/service/yiyuan-admin-dev/upload:/upload  -v /etc/localtime:/etc/localtime:ro -e TZ=Asia/Shanghai  --name yiyuan-admin-dev --restart always yiyuan-admin-dev:latest
echo '----部署java结束----'