...
docker run -d -p 40012:8085 --memory=512m --memory-swap=512m --privileged=true  -v /install/service/yiyuan-admin-dev/upload:/upload  -v /etc/localtime:/etc/localtime:ro -e TZ=Asia/Shanghai  --name yiyuan-admin-dev --restart always yiyuan-admin-dev:latest
echo '----部署java结束----'