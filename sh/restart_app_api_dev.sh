...
docker rmi yiyuan-app-api-dev:latest
docker build  -t yiyuan-app-api-dev:latest -f Dockerfile-dev  .
#docker run -it -v /宿主机目录:/容器目录 镜像名 宿主机必须执行该命令 echo "Asia/shanghai" > /etc/timezone
docker run -d -p 50013:8085 --memory=512m --memory-swap=512m --privileged=true  -v /install/service/yiyuan-app-api-dev/upload:/upload  -v /etc/localtime:/etc/localtime:ro -e TZ=Asia/Shanghai  --name yiyuan-app-api-dev --restart always yiyuan-app-api-dev:latest
echo '----部署java结束----'